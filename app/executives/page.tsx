...
export default function ExecutivesPage() {
  ...
  const checkAdminStatus = () => {
    ...
    if (password === "gucc-admin") { // This is just for demo purposes
      setIsAdmin(true);
      return true;
    }
    return false;
        ...
        <div className="mb-4 p-2 bg-yellow-50 border border-yellow-200 rounded-md">
          <div className="flex items-center justify-between">
            <div className="text-sm text-yellow-700">
              <span className="font-medium">Admin Mode</span> - You can {isResizeMode ? 'disable' : 'enable'} avatar resize mode
            </div>
            <Button 
                ...
                <section className="mb-12">
                  ...
                  <div className="grid gap-6 md:grid-cols-2 lg:grid-cols-3">
                    {facultyMembers.map((faculty, index) => (
                      <ExecutiveCard key={index} executive={faculty} isResizeMode={isResizeMode} />
                    ))}
                  </div>
                </section>
              )}
...
function ExecutiveCard({ 
  ...
  const [position, setPosition] = useState(executive.avatarPosition || { x: 0, y: 0 });
  ...
  const saveAvatarSettings = async () => {
    ...
    try {
      ...
      alert(`Error saving avatar settings: ${error instanceof Error ? error.message : 'Unknown error'}`);
    }
  };
    ...
    {isResizeMode && (
      <div className="p-4 bg-muted/20 rounded-b-lg">
        ...
        <div className="text-xs text-muted-foreground mb-3">
          Position: X:{position.x.toFixed(0)}, Y:{position.y.toFixed(0)} | Scale: {scale.toFixed(1)}
        </div>
